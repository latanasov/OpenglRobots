...
class RaceTrack {
    ...
    public Vector getLanePoint(int lane, double t) {
        if (null == controlPoints) {
            ...
            //Get the point on track
            point1 = this.getPoint(t, true);
            //Get the tangent (direction)
            point2 = this.getTangent(t).cross(point3);

            //Calculate the point on track by also  adding the multiplication  of the lane (1,2,3,4) and half lane width
            ...
            //If lane is 1
            if (lane == 1) {
                //Calculte the index based on time
                int index = lane1Index + Math.round((int) t);
                //If index is out of range start from the beginning of the track

                if (this.listLanePos1.size() < index) {
                    this.lane1Index = 0;
                }
                //Retrieve the position from the list of points
    ...
    public Vector getLaneTangent(int lane, double t) {
        if (null == controlPoints) {
            ...
            int index = lane1Index + Math.round((int) t);
            Vector tagentVector = new Vector(0, 0, 0);
            //Calculate the tangent by substracting the two "neighbour" points
